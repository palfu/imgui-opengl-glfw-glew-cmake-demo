...
void EditTransform(float *cameraView, float *cameraProjection, float *matrix, bool editTransformDecomposition)
{
    ...
    if (editTransformDecomposition) {
        ...
        if (ImGui::IsKeyPressed(ImGuiKey_V)) useSnap = !useSnap;
...
struct MySequence : public ImSequencer::SequenceInterface {
    ...
    virtual void CustomDraw(
        int index, ImDrawList *draw_list, const ImRect &rc, const ImRect &legendRect, const ImRect &clippingRect, const ImRect &legendClippingRect)
    {
...
void initTemplate(GraphEditor::Template & tmp, ImU32 const& head_color, ImU32 const& bg_color, ImU32 const& bg_over_color, ImU8 const& input_count, std::string const& input_name, ImU8 const& output_count, std::string const& output_name, ImU32* input_color=nullptr, ImU32* output_color=nullptr)
{
    tmp.mHeaderColor = head_color;
    tmp.mBackgroundColor = bg_color;
    tmp.mBackgroundColorOver = bg_over_color;
    tmp.mInputCount = input_count;
    tmp.mInputNames = nullptr;
    tmp.mOutputCount = output_count;
    tmp.mOutputNames = nullptr;
    tmp.mInputColors = input_color;
    tmp.mOutputColors = output_color;
}


void defaultInitTemplate(std::vector<GraphEditor::Template>& vec)
{
    GraphEditor::Template tmp1, tmp2;
    initTemplate(tmp1, IM_COL32(160, 160, 180, 255), IM_COL32(100, 100, 140, 255), IM_COL32(110, 110, 150, 255), 1, "MyInput",  1, "MyOutput0");
    initTemplate(tmp2, IM_COL32(250, 160, 180, 255), IM_COL32(100, 200, 140, 255), IM_COL32(110, 110, 50, 255), 1, "MyInput",  1, "MyOutput0");
...
struct GraphEditorDelegate : public GraphEditor::Delegate {
    GraphEditorDelegate(){
        defaultInitTemplate(mTemplates);
    }
    ...
    const GraphEditor::Node GetNode(GraphEditor::NodeIndex index) override
    {
        ...
        return GraphEditor::Node{myNode.name, myNode.templateIndex, ImRect(ImVec2(myNode.x, myNode.y), ImVec2(myNode.x + 200, myNode.y + 200)),
                                 myNode.mSelected};
    }
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (ImGui::CollapsingHeader("Sequencer")) {
            ...
            Sequencer(&mySequence, &currentFrame, &expanded, &selectedEntry, &firstFrame,
                      ImSequencer::SEQUENCER_EDIT_STARTEND | ImSequencer::SEQUENCER_ADD | ImSequencer::SEQUENCER_DEL | ImSequencer::SEQUENCER_COPYPASTE |
